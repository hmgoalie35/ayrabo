...
def parse_list(value, delimeter=','):
    return [item.strip() for item in value.split(delimeter)]


def parse_bool(value):
    """
    Given a string that represents a boolean, returns the actual boolean for that string.

    :param value: A string with value True, true, False, or false
    :return: A boolean for the value
    """
    value_lowered = value.lower()
    if value_lowered in ['true']:
        return True
    elif value_lowered in ['false']:
        return False
...
ALLOWED_HOSTS = parse_list(ENV_SETTINGS.get('ALLOWED_HOSTS', ''))
...
# EMAIL_USE_TLS = True
EMAIL_USE_SSL = parse_bool(ENV_SETTINGS.get('EMAIL_USE_SSL', 'false'))
...
MEDIA_URL = '/media/'
MEDIA_ROOT = ENV_SETTINGS.get('MEDIA_ROOT', os.path.join(BASE_DIR, 'media'))
