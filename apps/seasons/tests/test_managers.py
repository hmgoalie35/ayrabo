...
class SeasonManagerTests(BaseTestCase):
    def setUp(self):
        ...
        self.past_past_start_date = date(2015, 9, 4)
        ...
        # 2015-2016 season
        self.past_past_season = SeasonFactory(league=self.liahl, start_date=self.past_past_start_date,
                                              end_date=self.past_start_date)
        # 2016-2017 season
    ...
    def test_get_current_today_before_season_start(self, mock_date):
    ...
    @mock.patch('seasons.managers.date')
    def test_get_current_today_same_as_season_start(self, mock_date):
        mock_date.today.return_value = self.start_date
        self._assert_current_season()

    @mock.patch('seasons.managers.date')
    def test_get_current_today_after_season_start(self, mock_date):
        mock_date.today.return_value = self.start_date + timedelta(days=60)
        self._assert_current_season()

    @mock.patch('seasons.managers.date')
    def test_get_current_today_past_new_year(self, mock_date):
        mock_date.today.return_value = date(2018, 1, 1)
        self._assert_current_season()

    @mock.patch('seasons.managers.date')
    def test_get_current_today_same_as_season_end(self, mock_date):
        mock_date.today.return_value = self.end_date
        self.assertEqual(self._get_current_season().id, self.future_season.id)

    @mock.patch('seasons.managers.date')
    def test_get_current_today_after_season_end(self, mock_date):
        mock_date.today.return_value = self.end_date + timedelta(days=60)
        self.assertEqual(self._get_current_season().id, self.future_season.id)

    @mock.patch('seasons.managers.date')
    def test_get_past(self, mock_date):
        mock_date.today.return_value = self.end_date
