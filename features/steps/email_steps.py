...
@step(r'"(?P<username_or_email>[^"]*)" should receive an email with subject "(?P<subject>[^"]*)"')
...
@step(r'"(?P<username_or_email>[^"]*)" follows the email link with subject "(?P<subject>[^"]*)"')
def step_impl(context, username_or_email, subject):
...
@step(r'"(?P<username_or_email>[^"]*)" should have (?P<num_emails>\d+) emails?')
...
@step(r'I follow an invalid password reset link')
